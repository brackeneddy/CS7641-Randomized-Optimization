...
def solve_problem(problem, use_mimic=False):
    ...
    start_time = time.time()
    best_state_sa, best_fitness_sa, sa_curve = mlrose.simulated_annealing(
        problem,
    ...
    start_time = time.time()
    best_state_ga, best_fitness_ga, ga_curve = mlrose.genetic_alg(
        problem,
        pop_size=200,
        mutation_prob=0.1,
        max_attempts=100,
    ...
    if use_mimic:
        start_time = time.time()
        best_state_mimic, best_fitness_mimic, mimic_curve = mlrose.mimic(
            problem,
            pop_size=200,
            keep_pct=0.2,
            max_attempts=100,
...
for size in problem_sizes:
    ...
    problem_six_peaks = mlrose.DiscreteOpt(
        length=size, fitness_fn=fitness_six_peaks_fn, maximize=True, max_val=2
    )
    (
        six_peaks_rhc_curve,
        six_peaks_sa_curve,
        ...
        sa_time,
        ga_time,
    ) = solve_problem(problem_six_peaks)
    ...
    print(
        f"Six Peaks (Size: {size}) - RHC: {six_peaks_rhc_curve[-1, 0]}, SA: {six_peaks_sa_curve[-1, 0]}, "
        f"GA: {six_peaks_ga_curve[-1, 0]}"
    )

    # Continuous Peaks
    fitness_cont_peaks_fn = mlrose.ContinuousPeaks(t_pct=0.1)
    problem_cont_peaks = mlrose.DiscreteOpt(
        length=size, fitness_fn=fitness_cont_peaks_fn, maximize=True, max_val=2
    )
    (
        cont_peaks_rhc_curve,
        cont_peaks_sa_curve,
        cont_peaks_ga_curve,
        cont_peaks_mimic_curve,
        rhc_time,
        sa_time,
        ga_time,
        mimic_time,
    ) = solve_problem(problem_cont_peaks, use_mimic=True)
    ...
    print(
        f"Continuous Peaks (Size: {size}) - RHC: {cont_peaks_rhc_curve[-1, 0]}, SA: {cont_peaks_sa_curve[-1, 0]}, "
        f"GA: {cont_peaks_ga_curve[-1, 0]}, MIMIC: {cont_peaks_mimic_curve[-1, 0]}"
    )

# Calculate the number of weights for the neural network
num_features = X_train.shape[1]
num_hidden_nodes = 10  # Match A1 neural network architecture
num_output_nodes = len(np.unique(y_train))
num_weights = num_features * num_hidden_nodes + num_hidden_nodes * num_output_nodes


# Define a custom fitness function for NN weight optimization based on A1
...
problem_nn = mlrose.ContinuousOpt(
    length=num_weights, fitness_fn=fitness_nn, maximize=True, min_val=-1, max_val=1
)
...
for algo in times_six_peaks.keys():
    plt.plot(
        problem_sizes, times_six_peaks[algo], label=f"{algo} - Six Peaks", marker="o"
    )
...
for algo in times_cont_peaks.keys():
    plt.plot(
        problem_sizes,
        times_cont_peaks[algo],
        label=f"{algo} - Continuous Peaks",
        marker="x",
    )
...
for algo in fitness_six_peaks.keys():
    plt.plot(
        problem_sizes, fitness_six_peaks[algo], label=f"{algo} - Six Peaks", marker="o"
    )
...
best_fitness_values_peaks = {
    ...
    "Six Peaks": [
        fitness_six_peaks["RHC"][-1],
        fitness_six_peaks["SA"][-1],
        fitness_six_peaks["GA"][-1],
        None,
    ],
    "Continuous Peaks": [
        fitness_cont_peaks["RHC"][-1],
        fitness_cont_peaks["SA"][-1],
        fitness_cont_peaks["GA"][-1],
...
fitness_df_melted_peaks = fitness_df_peaks.melt(
    id_vars="Algorithm", var_name="Problem", value_name="Best Fitness"
)
fitness_df_melted_peaks["Problem"] = fitness_df_melted_peaks["Problem"].replace(
    {"0.0": "Six Peaks", "1.0": "Continuous Peaks"}
)

plt.figure(figsize=(12, 6))
sns.barplot(
    x="Problem", y="Best Fitness", hue="Algorithm", data=fitness_df_melted_peaks
)
...
# Fixing the issue with the x-axis labels
fitness_df_melted_nn = fitness_df_nn.melt(
    id_vars="Algorithm", var_name="Problem", value_name="Best Fitness"
)
...
# Comparison of A1 Neural Network accuracy to new algorithms
a1_accuracy = 0.9697  # Accuracy from A1 for Neural Network

# Filter out None values before taking the max
nn_weight_opt_best = [
    value for value in best_fitness_values_nn["NN Weight Opt."] if value is not None
]

comparison_data = {
    "Metric": ["Accuracy"],
    "A1 (Neural Network)": [a1_accuracy],
...
sns.barplot(
    x="Metric", y="value", hue="variable", data=pd.melt(comparison_df, id_vars="Metric")
)
plt.title("Comparison of Accuracy: A1 vs New Algorithms")
plt.ylabel("Accuracy")
plt.ylim(0, 1)
plt.show()
...
grid_search = GridSearchCV(
    MLPClassifier(max_iter=100), param_grid, cv=3, scoring="accuracy"
)
...
problem_nn_tuned = mlrose.ContinuousOpt(
    length=num_weights,
    fitness_fn=fitness_nn_tuned,
    maximize=True,
    min_val=-1,
    max_val=1,
)
...
nn_rhc_state_tuned, nn_rhc_fitness_tuned, nn_rhc_curve_tuned = solve_nn_problem(
    problem_nn_tuned, "RHC"
)
nn_sa_state_tuned, nn_sa_fitness_tuned, nn_sa_curve_tuned = solve_nn_problem(
    problem_nn_tuned, "SA"
)
nn_ga_state_tuned, nn_ga_fitness_tuned, nn_ga_curve_tuned = solve_nn_problem(
    problem_nn_tuned, "GA"
)
